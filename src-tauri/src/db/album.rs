...
pub fn update(conn: &Connection, id: &str, params_in: &UpdateAlbumParams) -> Result<Album> {
    ...
    if sets.is_empty() {
        return get_by_id(conn, id)?.ok_or_else(|| AppError::NotFound(format!("Album {id} not found")));
    }

    sets.push("updated_at = strftime('%Y-%m-%dT%H:%M:%SZ', 'now')".into());
    bind.push(Box::new(id.to_string()));
    let sql = format!("UPDATE albums SET {} WHERE id = ?{}", sets.join(", "), bind.len());
...
pub fn find_best_portrait(conn: &Connection, photo_ids: &[String]) -> Result<Option<String>> {
    if photo_ids.is_empty() { return Ok(None); }

    // 将 photo_ids 转为 (?1, ?2, ...) 占位符列表，供 IN 查询使用
    let placeholders: Vec<String> = photo_ids.iter().enumerate()
        .map(|(i, _)| format!("?{}", i + 1))
        .collect();
    let sql = format!(
    ...
    let params: Vec<&dyn rusqlite::types::ToSql> = photo_ids.iter()
    ...
    #[derive(Debug, Clone)]
    struct Candidate { id: String, score: i64 }
...
pub fn list_summaries(conn: &Connection) -> Result<Vec<AlbumSummary>> {
    ...
    let results = stmt.query_map([], row_to_summary)?
