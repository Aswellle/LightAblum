...
fn run_scan(
    ...
    let discovered_total = FileWalker::count(std::slice::from_ref(&folder));
    tracing::info!("[Scan] {path}: discovered {discovered_total} files");

    // walker.walk() 返回同步迭代器，整个循环无 .await，无 Send 问题
    for batch_result in walker.walk(std::slice::from_ref(&folder)) {
        ...
        let new_photos: Vec<NewPhoto> = to_add
            .par_iter()
            .filter_map(|entry| {
                ...
                let hash =
                    hasher::hash_file(&entry.path).unwrap_or_else(|_| Uuid::new_v4().to_string());
                let ext = entry
                    .path
                    .extension()
                    .and_then(|e| e.to_str())
                ...
                let created_at = exif
                    .created_at
                    .unwrap_or_else(|| entry.modified_at.format("%Y-%m-%dT%H:%M:%SZ").to_string());
                Some(NewPhoto {
                    file_path,
        ...
        let updated_photos: Vec<NewPhoto> = to_update
            .par_iter()
            .filter_map(|entry| {
                let mut exif = exif_meta::extract(&entry.path).unwrap_or_default();
                exif_meta::enrich_dimensions(&mut exif, &entry.path);
    ...
    let missing_ids = match db.get() {
        ...
        Err(e) => {
            tracing::error!("[Scan] DB pool during missing detection: {e}");
            vec![]
        }
    };
    ...
    let removed_paths: Vec<String> = if !missing_ids.is_empty() {
        tracing::info!(
            "[Scan] {path}: {} files missing from disk",
            missing_ids.len()
        );
        match db.get() {
            Err(e) => {
                tracing::error!("[Scan] DB pool during soft-delete: {e}");
                vec![]
            }
            Ok(conn) => {
